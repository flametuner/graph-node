...
impl EntityCache {
    ...
    pub fn get(&mut self, eref: &EntityKey) -> Result<Option<Entity>, s::QueryExecutionError> {
        ...
        let mut entity = self.current.get_entity(&*self.store, &EntityMultiKey::Equal(eref.clone()))?;
    ...
    pub fn as_modifications(mut self) -> Result<ModificationsAndCache, s::QueryExecutionError> {
        ...
        let missing = self
            .updates
            .keys()
            .filter(|key| !self.current.contains_key(&EntityMultiKey::Equal((*key).clone())));
        ...
        for (entity_key, entity) in self.store.get_many(missing.cloned().collect())? {
            self.current.insert(EntityMultiKey::Equal(entity_key), Some(entity));
        }
